document.addEventListener('DOMContentLoaded', () => {
  ...
  eventForm?.addEventListener('submit', (e) => {
    ...
    const eventData = {
      id: editRow ? getRowData(editRow).id : makeEventId(),
      title,
      date,
      time,
      quota,
      ...
      bloodTypes,
      description,
      status: editRow ? getRowData(editRow).status : 'Upcoming',
      registered: editRow ? getQuotaNumbers(editRow).registered : 0
    };

    if (editRow) {
      updateExistingRow(editRow, eventData);
      showToast('Event updated successfully.', 'success');
    } else {
      const newRow = createEventRow(eventData);
  ...
  function applyFilters(showNoResultToast = true) {
    ...
    if (visible === 0 && showNoResultToast) {
      showToast('No matching events found.', 'info');
    }
  }
